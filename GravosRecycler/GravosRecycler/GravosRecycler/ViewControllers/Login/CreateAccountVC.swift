				...
				if(self.isValidEmail(testStr: self.txtEmail.text!)){
					if(self.isValidName(self.txtFirstName.text!) && self.isValidName(self.txtLastName.text!)){
						Alamofire.request(URL_USER_REGISTER,method: .post,parameters:parameters).validate().responseString{
							...
							let dict = self.convertStringToDictionary(text: response.result.value!)
							print("Convert to dict \(dict)")
							}
						...
						//						let postString = "email=\(txtEmail.text!)&firstname=\(txtFirstName.text!)&lastname=\(txtLastName.text!)&password=\(txtPassword.text!)&contactnumber=\(txtContactNumber.text!)&address=\(txtaddress.text!)";
						//
						//						print(postString);
						//
